    ...
    <div className="sm:w-[70%] lg:w-[50%] mx-auto w-full ">
     ...
     <div>
      {CurrentPosts.map((item:post) => {
        return <PostCard _id={item._id} author={item.author} category={item.category} content={item.content} createdAt={item.createdAt} likes={item.likes} media={item.media} postedBy={item.postedBy}title={item.title} key={item._id}  updatedAt={item.updatedAt}/>
      }
      )}
     </div>
    </div>
